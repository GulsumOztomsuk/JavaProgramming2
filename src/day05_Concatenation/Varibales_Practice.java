package day05_Concatenation;

public class Varibales_Practice {
    public static void main(String[] args) {

        byte num1=100;
        short num2=10000;  //ınt double float long
        double num3=3.5;
        float num4=2.5f;
        long num5=999999999999L;
        int num6=850000;   // double long float

        System.out.println("num1 = " + num1);
        System.out.println("num2 = " + num2);
        System.out.println("num3 = " + num3);
        System.out.println("num4 = " + num4);
        System.out.println("num5 = " + num5);
        System.out.println("num6 = " + num6);
    }

}

/*
"123" + 1= 1231   concatenation because "123" is a string not number
123 + 1= 124     addination 123 is a number
 */


/*
1. create a class called Variables_Practice & declare the following variables:
                            num1 = 100;
                            num2 = 10000;
                            num3 = 3.5;
                            num4 = 2.5f;
                            num5 = 999999999999L;
                            num6 = 850000;
                print each of the variables above on the console
 */